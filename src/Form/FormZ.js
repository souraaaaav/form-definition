import { Box, Button, Container, Grid, IconButton, Modal, TextField, Typography, makeStyles } from '@material-ui/core';
...
const Form = () => {
    ...
    const initialSectionState = { code_name: '', name: '', row_position: '', column_position: '', dp_list: [] };
    ...
    const initialDataPointState = { name: '', label: '', editable: '', row_position: '', column_position: '', position_relative_to_parent: '', sub_data_point_list: [] };
    const [newDatapointData, setNewDatapointData] = useState(initialDataPointState);
    ...
    const initialSubDataPointState = { name: '', label: '', editable: '', row_position: '', column_position: '', position_relative_to_parent: '', sub_data_point_list: [] };
    const [newSubDatapointData, setNewSubDatapointData] = useState(initialSubDataPointState);
            ...
            <Box sx={style}>
                <form onSubmit={handleSubmit} style={{ margin: '0 auto' }}>
                    <TextField
                        ...
                        onChange={(e) => handleChange(e.target.getAttribute('wti'), e.target.value)}
                        fullWidth
                        margin="normal"
                    />
                    <TextField
        ...
        <Modal open={openSectionModal} onClose={handleSectionCloseModal}>
            <Box sx={style}>
                ...
                <TextField
                    ...
                    onChange={(e) => setNewSectionData({ ...newSectionData, code_name: e.target.value })}
                    fullWidth
                />
                <TextField
                    label="Name"
        ...
        <Modal open={openDatapointModal} onClose={handleDatapointCloseModal}>
            <Box sx={style}>
                ...
                <TextField
                    ...
                    onChange={(e) => setNewDatapointData({ ...newDatapointData, label: e.target.value })}
                    fullWidth
                />
                <Button
                    variant="contained"
    ...
    const getCurrentPathSubDP = (givenPath) => {
        ...
        while (currentPath.length > 1) {
            ...
            if (counter === 1) {
                ...
                tempInsertedDataPoint = tempInsertedDataPoint[currentLevel['code_name']];
                tempSectionDpWiseSubDp = tempSectionDpWiseSubDp[currentLevel['code_name']];
            }
            if (counter >= 3 && counter % 2 !== 0) {
                //going through sub_data_point_list
                if (counter >= 5) {
                    subDpDict = tempInsertedDataPoint.sub_data_point_list[currentLevel['name']];
                    tempInsertedDataPoint = tempInsertedDataPoint.sub_data_point_list[currentLevel['name']];
        ...
        const result = {};
        if (Object.keys(tempSectionDpWiseSubDp).length >= 1 && Object.keys(subDpDict).length >= 1) {
    ...
    const AddSubDatapointComponent = () => {
        ...
        return (
            <Modal open={openSubDatapointModal} onClose={handleSubDatapointCloseModal}>
                <Box sx={style}>
                    <h1>Add Datapoint</h1>
                    <Autocomplete
                        // freeSolo
                        // autoSelect
                        options={getCurrentPathSubDP(subDataPointPath) ? Object.keys(getCurrentPathSubDP(subDataPointPath)) : null}
                        renderInput={(params) => (
                            <TextField {...params} label="Name" variant="outlined" />
                        )}
                    ...
                    <TextField
                        ...
                        onChange={(e) => setNewSubDatapointData({ ...newSubDatapointData, label: e.target.value })}
                        fullWidth
                    />
                    <Button
                        variant="contained"
                ...
                if (Array.isArray(value)) {
                    ...
                    return (
                        <div key={key} className={classes.sectionContainer}>
                            ...
                            {currentPath.length > 3 && currentPath[currentPath.length - 1] === 'sub_data_point_list' && checkPathHaveSubDPInForm(currentPath) && (
                                <Button
                                    className={classes.addButton}
                                    variant="contained"
                                    color="primary"
                            ...
                            {!isSectionCollapsed && (
                                <Grid container spacing={2}>
                                    <Drop id={currentPath.join(',')} type={currentPath.join(',')}>
                                            ...
                                            <Grid item xs={12} md={12} key={`${key}-${index}`}>
                                                ...
                                                >
                                                    <div className={classes.nestedSectionContainer}>
                                                        {renderForm(section, collapseStates, setCollapseStates, [...currentPath, index])}
                                                    </div>
                                                </Drag>
                                            </Grid>
                                        ))}
                        ...
                        <div key={key} className={classes.sectionContainer} test={key}>
                            {!isSectionCollapsed && (
                                <Grid container spacing={2}>
                                    ...
                                    {renderForm(value, collapseStates, setCollapseStates, currentPath)}
                                </Grid>
                            )}
                        </div>
                    );
                    ...
                    return (
                        <>
                            {key !== "id" ? (<Grid item xs={12} key={key}>
                                <TextField
                                    label={key}
                                    name={key}
                                    defaultValue={value === null ? 'null' : value}
                                    fullWidth
                                    onChange={(e) => handleTextUpdate(path, key, e.target.value)}
                                />
                            </Grid>) : null}
                        </>
                    );
